...
const mobyGamesCache = JSON.parse(fs.readFileSync('cache/gallery/MobyGames.json', 'utf-8'));
function saveGalleryCache() {
    fs.writeFileSync('cache/gallery/LemonAmiga.json', JSON.stringify(lemonAmigaCache, null, 2));
    fs.writeFileSync('cache/gallery/MobyGames.json', JSON.stringify(mobyGamesCache, null, 2));
}
...
const getGallery = {
    ...
    'MobyGames': async ({ url, gallerySection }) => {
        if (mobyGamesCache[url]) return mobyGamesCache[url];
        gallerySection ??= 'DOS screenshots';
        const gameId = url.match(/game\/(\d+)\//)?.[1] ?? url.match(/game\/dos\/(.+)/)?.[1];
        const html = gameId && fs.existsSync(`../../cache/gallery/${gameId}.htm`)
            ? fs.readFileSync(`../../cache/gallery/${gameId}.htm`, 'utf-8')
            : await (await fetch(`${url}/screenshots`)).text();
        const doc = new dom({ errorHandler: () => {} }).parseFromString(html, 'text/html');
        const gal = xpath.select1(`//*[name() = 'h2' and normalize-space() = '${gallerySection}']/following-sibling::*[contains(@class, 'img-holder')]`, doc);
        const gallery = gal && xpath.select("./*[name() = 'figure']", gal).map(figure => xpath.select1("string(.//*[name() = 'img']/@src)", figure));
        mobyGamesCache[url] = gallery;
        return gallery;
    },
};
